...
class MeetingValidatorTest {
  ...
  void validate_StartAndEndTimeAreEqual_ShouldRejectEndTime() {
    ...
    Meeting meeting =
        Meeting.builder().title("Team Meeting").date(LocalDate.now()).start(time).end(time).build();
  ...
  private void verifyErrors(String field, String errorCode, String message) {
    ...
    long hits =
        fieldErrors.stream()
            .flatMap(elem -> Arrays.stream(elem.getCodes()).filter(errorCodePredicate))
            .count();
    assertThat(hits).isGreaterThanOrEqualTo(1L);

    boolean found =
        fieldErrors.stream().anyMatch(elem -> messagePredicate.test(elem.getDefaultMessage()));
